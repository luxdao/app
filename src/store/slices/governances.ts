...
export type SetGovernorGovernancePayload = {
...
export type GovernancesSlice = {
  ...
  setGovernorGovernance: (daoKey: DAOKey, payload: SetGovernorGovernancePayload) => void;
  ...
  setProposal: (daoKey: DAOKey, proposal: GovernorProposal) => void;
...
> = (set, get) => ({
  ...
  setGovernorGovernance: (daoKey, payload) => {
    set(
      state => {
        if (!state.governances[daoKey]) {
          state.governances[daoKey] = {
  ...
  setProposalVote: (daoKey, proposalId, votesSummary, proposalVote) => {
    set(
      state => {
        const governorProposal = state.governances[daoKey].proposals?.find(
          (p: any) => p.proposalId === proposalId,
        ) as GovernorProposal;
        if (!governorProposal) {
          return;
        ...
        const existingVoteIndex = governorProposal.votes.findIndex(
          v => v.voter === proposalVote.voter,
        );
        if (existingVoteIndex !== -1) {
          governorProposal.votes[existingVoteIndex] = proposalVote;
        } else {
          (governorProposal.votes as (ProposalVote | ERC721ProposalVote)[]).push(proposalVote);
        }
        governorProposal.votesSummary = votesSummary;
      },
      false,
      'setProposalVote',
    );
  ...
  setGovernanceAccountData: (daoKey, governanceAccountData) => {
    set(
      state => {
        const governorGovernance = state.governances[daoKey];

        if (
          !state.governances[daoKey] ||
          !state.governances[daoKey].isGovernor ||
          !governorGovernance.votesToken
        ) {
          return;
        }

        governorGovernance.votesToken.balance = governanceAccountData.balance;
        governorGovernance.votesToken.delegatee = governanceAccountData.delegatee;
      },
      false,
      'setGovernanceAccountData',
    );
