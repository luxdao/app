...
import { getProposalVotesSummary, mapProposalCreatedEventToProposal } from '../../utils/governor';
...
export function useGovernanceListeners({
  ...
  onProposalCreated: (proposal: GovernorProposal) => void;
  ...
  useEffect(() => {
    ...
    const unwatchProposalCreated = governorContract.watchEvent.ProposalCreated({
      onLogs: async logs => {
    ...
    const unwatchProposalExecuted = governorContract.watchEvent.ProposalExecuted({
      onLogs: async logs => {
        for (const log of logs) {
