...
export function useGovernanceFetcher() {
  ...
  const fetchDAOGovernance = useCallback(
    async ({
      ...
      onGovernorGovernanceLoaded: (governance: SetGovernorGovernancePayload) => void;
      onProposalsLoaded: (proposals: FractalProposal[]) => void;
      onProposalLoaded: (proposal: GovernorProposal, index: number, totalProposals: number) => void;
      ...
      const governorModule = getGovernorModuleFromModules(daoModules);
      clearIntervals();
      if (!governorModule) {
        onMultisigGovernanceLoaded();
        ...
        const [strategiesAddresses, nextStrategy] = await governorContract.read.getStrategies([
          SENTINEL_ADDRESS,
          3n,
        ]);
        ...
        if (strategies.length > 0) {
          ...
          let linearVotingErc20WithRolesWhitelistingAddress = strategies.find(strategy => {
          ...
          let linearVotingErc721WithRolesWhitelistingAddress = strategies.find(strategy => {
            return strategy.type == FractalTokenType.erc721 && strategy.withWhitelist == true;
          })?.address;

          const erc20VotingStrategyAddress =
            linearVotingErc20Address || linearVotingErc20WithRolesWhitelistingAddress;

          const erc721VotingStrategyAddress =
            linearVotingErc721Address || linearVotingErc721WithRolesWhitelistingAddress;
          ...
          if (votesTokenAddress && erc20VotingStrategyAddress) {
            ...
            onGovernorGovernanceLoaded({
              moduleGovernorAddress: governorContract.address,
              votesToken,
              ...
              isGovernor: true,
              lockedVotesToken,
              type: GovernanceType.GOVERNOR_ERC20,
            });
            ...
            const executedEvents = await governorContract.getEvents.ProposalExecuted({
              fromBlock: 0n,
            });
            const proposalCreatedEvents = (
              await governorContract.getEvents.ProposalCreated({ fromBlock: 0n })
            ).reverse();
            ...
            const [
              ...
              governorContract.read.timelockPeriod(),
              governorContract.read.executionPeriod(),
            ]);
            ...
            onGovernorGovernanceLoaded({
              ...
              votingStrategy,
              isGovernor: true,
              type: GovernanceType.GOVERNOR_ERC721,
            });
  ...
  const fetchGaslessVotingDAOData = useCallback(
    ...
    [publicClient, accountAbstraction, moduleProxyFactory, daoPaymasterV1MasterCopy],
  );
  ...
  const fetchStakingDAOData = useCallback(
    ...
    [governance, publicClient, votesERC20StakedV1MasterCopy, moduleProxyFactory],
  );
