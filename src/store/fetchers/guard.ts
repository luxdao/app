...
export function useGuardFetcher() {
  ...
  const fetchDAOGuard = useCallback(
    async ({
      ...
      _governorModule?: DAOModule;
    }) => {
      if (_governorModule) {
        const governorContract = getContract({
          abi: legacy.abis.Governor,
          address: _governorModule.moduleAddress,
          client: publicClient,
        });

        const governorGuardAddress = await governorContract.read.getGuard();
        const { isFreezeGuardGovernor } = await getAddressContractType(governorGuardAddress);

        if (governorGuardAddress === zeroAddress || !isFreezeGuardGovernor) {
          return;
        }

        const freezeGuardContract = getContract({
          abi: legacy.abis.GovernorFreezeGuard,
          address: governorGuardAddress,
          client: publicClient,
        });
        ...
        return {
          freezeGuardContractAddress: governorGuardAddress,
          freezeVotingContractAddress: freezeVotingAddress,
          freezeVotingType,
          freezeGuardType: FreezeGuardType.GOVERNOR,
          ...freezeGuard,
        };
