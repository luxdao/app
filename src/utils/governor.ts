...
export const getGovernorProposalState = async (
  governorContract: GetContractReturnType<typeof legacy.abis.Governor, PublicClient>,
  proposalId: number,
): Promise<FractalProposalState> => {
  const state = await governorContract.read.proposalState([proposalId]);
...
const getQuorum = async ({
  ...
  if (
    ...
    strategyType === VotingStrategyType.LINEAR_ERC20_ROLES_WHITELISTING
  ) {
    quorum = await (
    ...
    strategyType === VotingStrategyType.LINEAR_ERC721_ROLES_WHITELISTING
  ) {
    quorum = await (
...
export const mapProposalCreatedEventToProposal = async (
  ...
  erc721VotedEvents:
    ...
    | undefined,
  executedEvents:
    | GetContractEventsReturnType<typeof legacy.abis.Governor, 'ProposalExecuted'>
    | undefined,
  data?: ProposalData,
) => {
  let proposalVotes = {
  ...
  const state = await getGovernorProposalState(governorContract, proposalId);
...
export function getGovernorModuleFromModules(modules: DAOModule[]) {
  return modules.find(module => module.moduleType === FractalModuleType.GOVERNOR);
}
