...
export function usePrepareFormData() {
  ...
  const prepareGovernorERC20FormData = useCallback(
    async ({
      tokenSupply,
      tokenAllocations,
      ...
      maxTotalSupply,
      ...rest
    }: GovernorERC20DAO<BigIntValuePair> & FreezeGuardConfigParam): Promise<
      GovernorERC20DAO | undefined
    > => {
  ...
  const prepareGovernorERC721FormData = useCallback(
    async ({
      quorumPercentage,
      timelock,
      executionPeriod,
      ...
      quorumThreshold,
      ...rest
    }: GovernorERC721DAO<BigIntValuePair> & FreezeGuardConfigParam): Promise<
      GovernorERC721DAO | undefined
    > => {
      let freezeGuardData;
      if (freezeGuard) {
        freezeGuardData = await prepareFreezeGuardData(freezeGuard);
  ...
  return {
    ...
    prepareGovernorERC20FormData,
    prepareGovernorERC721FormData,
    checkVotesToken,
  };
}
