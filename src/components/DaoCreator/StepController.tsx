...
import { GovernorGovernance } from './formComponents/GovernorGovernance';
import GovernorNFTDetails from './formComponents/GovernorNFTDetails';
import { GovernorTokenDetails } from './formComponents/GovernorTokenDetails';
...
function StepController(props: Omit<ICreationStepProps, 'steps'>) {
  ...
  const steps = useMemo(() => {
    ...
    const isERC20 = values.essentials.governance === GovernanceType.GOVERNOR_ERC20;
    const isERC721 = values.essentials.governance === GovernanceType.GOVERNOR_ERC721;
  ...
  return (
    <Routes>
      ...
      {steps.includes(CreatorSteps.GOVERNOR_DETAILS) && (
        <Route
          path={CreatorSteps.GOVERNOR_DETAILS}
          element={
            <GovernorGovernance
              {...props}
              steps={steps}
