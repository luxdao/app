...
export function GovernorGovernance(props: ICreationStepProps) {
  ...
  useEffect(() => {
    ...
    setFieldValue('governor.votingPeriod', { bigintValue: minutes, value: minutes.toString() });
  }, [setFieldValue, votingPeriodDays]);

  // same for timelock and execution period
  useEffect(() => {
    const minutes = timelockPeriodDays * 24 * 60;
    setFieldValue('governor.timelock', { bigintValue: minutes, value: minutes.toString() });
  }, [setFieldValue, timelockPeriodDays]);

  useEffect(() => {
    const minutes = executionPeriodDays * 24 * 60;
    setFieldValue('governor.executionPeriod', { bigintValue: minutes, value: minutes.toString() });
  ...
  return (
    <>
      ...
      >
        ...
        >
          {/* QUORUM */}
          {values.governor.votingStrategyType === VotingStrategyType.LINEAR_ERC20 ? (
            <LabelComponent
            ...
            >
              <InputGroup>
                <BigIntInput
                  value={values.governor.quorumPercentage.bigintValue}
                  onChange={valuePair => setFieldValue('governor.quorumPercentage', valuePair)}
                  max="100"
                  decimalPlaces={0}
