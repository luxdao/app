...
export function InfoGovernance() {
  ...
  const governanceGovernor = governance.isGovernor ? (governance as GovernorGovernance) : null;
  ...
  useEffect(() => {
    const setTimelockInfo = async () => {
      ...
      } else if (governanceGovernor !== null) {
        const timelock = governanceGovernor.votingStrategy?.timeLockPeriod;
        if (timelock?.formatted) {
          setTimelockPeriod(timelock.formatted);
        }
        // TODO Governor execution period
  ...
  return (
    <Box data-testid="dashboard-daoGovernance">
      ...
      {governanceGovernor?.votingStrategy?.votingPeriod && (
        <Flex
          alignItems="center"
          justifyContent="space-between"
          mb="0.25rem"
        ...
        >
          ...
          <Text textAlign="right">{governanceGovernor.votingStrategy.votingPeriod.formatted}</Text>
        </Flex>
      )}
      {governanceGovernor?.votingStrategy?.quorumPercentage && (
        <Flex
          alignItems="center"
          justifyContent="space-between"
          mb="0.25rem"
        ...
        >
          ...
          <Text textAlign="right">
            {`${governanceGovernor.votingStrategy.quorumPercentage.formatted}%`}
          </Text>
        </Flex>
      )}
      {governanceGovernor?.votingStrategy?.quorumThreshold && (
        <Flex
          alignItems="center"
          justifyContent="space-between"
          mb="0.25rem"
        ...
        >
          <Text color="color-neutral-300">{t('titleQuorum')}</Text>
          <Text textAlign="right">
            {governanceGovernor.votingStrategy.quorumThreshold.formatted}
          </Text>
        </Flex>
      )}
      {timelockPeriod && (
