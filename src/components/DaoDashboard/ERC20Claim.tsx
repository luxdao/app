...
import { GovernorGovernance } from '../../types';
...
export function ERCO20Claim() {
  ...
  const claimToken = async () => {
    if (!tokenClaimContractAddress || !governorGovernance.votesToken || !account || !walletClient) {
      return;
    }
    ...
    contractCall({
      ...
      pendingMessage: t('pendingTokenClaim', {
        symbol: governorGovernance.votesToken.symbol,
        ns: 'transaction',
      }),
  ...
  if (!governorGovernance.votesToken || userClaimable === 0n) return null;
  const claimableString = formatCoin(
    userClaimable,
    false,
    governorGovernance.votesToken.decimals,
