...
function ProposalVotes({ proposal }: { proposal: GovernorProposal }) {
  const {
    proposalId,
    votesSummary: { yes, no, abstain },
    votes,
  ...
  const isERC20 = useMemo(
    () => governorGovernance.type === GovernanceType.GOVERNOR_ERC20,
    [governorGovernance.type],
  );
  const isERC721 = useMemo(
    () => governorGovernance.type === GovernanceType.GOVERNOR_ERC721,
    [governorGovernance.type],
  );
  ...
  if ((isERC20 && !governorGovernance.votesToken) || (isERC721 && !governorGovernance.erc721Tokens)) {
    return (
      <Box mt={4}>
        <InfoBoxLoader />
      </Box>
              ...
              {votes.map(vote => {
                return (
                  <ProposalERC20VoteItem
                    key={vote.voter}
                    vote={vote}
                    govTokenTotalSupply={governorGovernance.votesToken?.totalSupply || 0n}
                    govTokenDecimals={governorGovernance.votesToken?.decimals || 0}
                    govTokenSymbol={governorGovernance.votesToken?.symbol || ''}
                  />
                );
              })}
            </Grid>
