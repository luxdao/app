...
export function CastVote({ proposal }: { proposal: FractalProposal }) {
  ...
  const { castVote, castVotePending, castGaslessVote, castGaslessVotePending, canCastGaslessVote } =
    useCastVote(proposal.proposalId, governorProposal.votingStrategy);
  ...
  const proposalStartBlockNotFinalized = Boolean(
    !snapshotProposal &&
      ...
      currentBlockNumber &&
      governorProposal.startBlock >= currentBlockNumber,
  );

  const disabled =
    castVotePending ||
    castGaslessVotePending ||
    governorProposal.state !== FractalProposalState.ACTIVE ||
