...
export function RolesTable({
  ...
  handleRoleClick: (roleId: Address) => void;
  rolesTree: DAOTree;
}) {
  return (
    <Box
...
export function RolesEditTable({ handleRoleClick }: { handleRoleClick: (roleId: Hex) => void }) {
  const { rolesTree, getHat } = useRolesStore();
  const { values, setFieldValue } = useFormikContext<RoleFormValues>();
  if (rolesTree === undefined) {
    return <RoleCardLoading />;
  }
  if (rolesTree === null && !values.hats.length) {
    return (
      <NoDataCard
