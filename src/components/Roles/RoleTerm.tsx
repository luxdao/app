...
export default function RoleTerm({
  ...
  const { rolesTree, getHat, updateCurrentTermStatus } = useRolesStore();
  const { data: walletClient } = useNetworkWalletClient();
  const { t } = useTranslation(['roles']);
  const {
    contracts: { rolesProtocol },
  } = useNetworkConfigStore();

  const roleHat = useMemo(() => {
    if (!roleId) return undefined;
    return getHat(roleId);
  }, [getHat, roleId]);
    ...
    contractCall({
      contractFn: () => {
        const daoAutonomousAdminContract = getContract({
          ...
          address: adminRoleWearer,
          client: walletClient,
        });
        return daoAutonomousAdminContract.write.triggerStartNextTerm([
          {
            currentWearer: wearerAddress,
            hatsProtocol: rolesProtocol, // Map config name to contract expected field
            roleId: BigInt(roleHat.id),
