...
import { RoleFormValues, RoleFormValue } from '../../../types/roles';
import { DevSet10MinPaymentStream } from '../../../utils/dev/DevSet10MinPaymentStream';
import { DatePicker } from '../../ui/forms/DatePicker';
import { ModalType } from '../../ui/modals/ModalProvider';
import { useDAOModal } from '../../ui/modals/useDAOModal';
...
export function RoleFormPaymentStream({ formIndex }: { formIndex: number }) {
  ...
  const roleEditingPaymentsErrors = (errors.roleEditing as FormikErrors<RoleFormValue>)
    ?.payments;
  const roleId = values.roleEditing?.id;
  ...
  const existingPayment = useMemo(
    () => (!!streamId && !!roleId ? getPayment(roleId, streamId) : undefined),
    [roleId, streamId, getPayment],
  );
