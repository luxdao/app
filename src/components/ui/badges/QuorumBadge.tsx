...
import { GovernorGovernance, GovernorProposal } from '../../../types';

export default function QuorumBadge({ proposal }: { proposal: GovernorProposal }) {
  const { daoKey } = useCurrentDAOKey();
  const { governance } = useDAOStore({ daoKey });
  const { t } = useTranslation('common');

  const governorGovernance = governance as GovernorGovernance;
  const { votesToken, erc721Tokens, votingStrategy } = governorGovernance;
