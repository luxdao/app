...
export function SafeSettingsModal({
  ...
  const handleEditGovernorGovernance = async (updatedValues: SafeSettingsEdits) => {
    if (!updatedValues.governor) {
      throw new Error('Governor settings are not set');
    }

    if (!moduleGovernorAddress) {
      throw new Error('Governor module address is not set');
    }

    const { quorumPercentage, quorumThreshold, votingPeriod, timelockPeriod, executionPeriod } =
      updatedValues.governor;
  ...
  const submitAllSettingsEditsProposal = async (values: SafeSettingsEdits) => {
    ...
    resetActions();
    const { general, multisig, governor, permissions, paymasterGasTank, token, staking } = values;
    if (general) {
    ...
    if (governor) {
      const { action, title } = await handleEditGovernorGovernance(values);
