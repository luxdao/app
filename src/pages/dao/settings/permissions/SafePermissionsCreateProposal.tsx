...
import { useDAOModal } from '../../../../components/ui/modals/useDAOModal';
...
export function SafePermissionsCreateProposal() {
  ...
  const [proposerThreshold, setProposerThreshold] = useState<BigIntValuePair>({
    bigintValue: BigInt(governorGovernance.votingStrategy?.proposerThreshold?.value ?? 0),
    value: governorGovernance.votingStrategy?.proposerThreshold?.formatted ?? '0',
  });

  useEffect(() => {
    if (governorGovernance.votingStrategy?.proposerThreshold) {
      setProposerThreshold({
        bigintValue: BigInt(governorGovernance.votingStrategy.proposerThreshold.value),
        value: governorGovernance.votingStrategy.proposerThreshold.formatted ?? '0',
      });
    }
  }, [governorGovernance.votingStrategy?.proposerThreshold]);
  ...
  const handleCreateProposal = async () => {
    if (proposerThreshold.bigintValue !== undefined && moduleGovernorAddress) {
      ...
      if (linearVotingErc20Address) {
      ...
      } else if (linearVotingErc20WithRolesWhitelistingAddress) {
        ...
        const { votesToken, votingStrategy } = governorGovernance;
      ...
      } else if (linearVotingErc721WithRolesWhitelistingAddress) {
        actionType = ProposalActionType.ADD;
        const strategyNonce = getRandomBytes();

        const { erc721Tokens, votingStrategy } = governorGovernance;
