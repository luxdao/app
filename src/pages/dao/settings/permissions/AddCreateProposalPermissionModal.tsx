...
import { GovernorGovernance, BigIntValuePair } from '../../../../types';
...
export function AddCreateProposalPermissionModal({
  ...
  const proposerThresholdFromGovernanceObject = (governanceObject: GovernorGovernance) => {
    return {
  ...
  const [existingProposerThreshold, setExistingProposerThreshold] = useState<BigIntValuePair>(
    proposerThresholdFromGovernanceObject(governorGovernance),
  );

  useEffect(() => {
    setExistingProposerThreshold(proposerThresholdFromGovernanceObject(governorGovernance));
  }, [governorGovernance]);
