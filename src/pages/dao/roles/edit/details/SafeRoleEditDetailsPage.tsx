...
export function SafeRoleEditDetailsPage() {
  ...
  const hatEditingId = searchParams.get('roleId');
  ...
  const roleIndex = values.hats.findIndex(h => h.id === hatEditingId);
  ...
  const handleRemoveRole = () => {
    ...
    if (roleIndex !== -1) {
      if (role?.editedRole?.status === EditBadgeStatus.New) {
        ...
        setFieldValue(`hats.${roleIndex}`, { ...values.hats[roleIndex], editedRole });
      }
    }
  ...
  return (
    <>
      <FieldArray name="hats">
        {({ push }: { push: (roleHatFormValue: RoleFormValue) => void }) => (
          <>
            <PageHeader
              breadcrumbs={[
                {
