...
const useDeployGovernor = () => {
  ...
  const deployGovernor = useCallback(
    async (
      daoData: GovernorERC20DAO | GovernorERC721DAO | SubDAO,
      ...
      if (subgraphInfo?.parentAddress) {
        ...
        if (!loadedParentModule) {
          ...
          const parentGovernorModule = parentModules.find(
            fractalModule => fractalModule.moduleType === FractalModuleType.GOVERNOR,
          );
          if (parentGovernorModule) {
            const governorContract = getContract({
              abi: legacy.abis.Governor,
              address: parentGovernorModule.moduleAddress,
              client: publicClient,
            });

            // @dev assumes the first strategy is the voting contract
            const strategies = await governorContract.read.getStrategies([SENTINEL_ADDRESS, 0n]);
      ...
      const safeTx = await daoTxBuilder.buildGovernorTx({
        shouldSetName,
        shouldSetSnapshot,
      ...
      await submitProposal({
        ...
        pendingToastMessage: t('modifyGovernanceSetGovernorProposalPendingMessage'),
  ...
  return deployGovernor;
};

export default useDeployGovernor;
