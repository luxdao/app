...
interface ISubmitGovernorProposal extends ISubmitProposal {
...
export default function useSubmitProposal({
  ...
  const globalGovernorContract = useMemo(() => {
    if (!modules || !walletClient) {
      return;
    }
    const governorModule = getGovernorModuleFromModules(modules);
    if (!governorModule) {
      return;
    }
  ...
  const submitGovernorProposal = useCallback(
    async ({
      proposalData,
      governorAddress,
      ...
      try {
        ...
        // @todo: Implement voting strategy proposal selection when/if we will support multiple strategies on single Governor instance
        const txHash = await governorContract.write.submitProposal([
          votingStrategyAddress,
          '0x',
          transactions,
          JSON.stringify({
        ...
        toast.error(failedToastMessage, { id: toastId });
        logError(e, 'Error during Governor proposal creation');
      } finally {
        setPendingCreateTx(false);
      }
    },
  ...
  const submitProposal: SubmitProposalFunction = useCallback(
    async ({
      ...
      if (safeAddress && isAddress(safeAddress)) {
        ...
        if (!governorModule || !votingStrategies) {
          ...
          await submitGovernorProposal({
            ...
            successCallback,
            safeAddress,
            governorAddress: governorModule.moduleAddress,
        ...
        if (!globalGovernorContract || !votingStrategyAddress) {
          await submitMultisigProposal({
            proposalData,
            pendingToastMessage,
            successToastMessage,
          ...
          await submitGovernorProposal({
            ...
            governorAddress: globalGovernorContract.address,
            safeAddress: safe?.address,
          });
        }
      }
    ...
    [
      ...
      freezeVotingContractAddress,
      getVotingStrategies,
      globalGovernorContract,
      lookupModules,
      linearVotingErc20Address,
      linearVotingErc20WithRolesWhitelistingAddress,
      safe?.address,
      safeAPI,
      submitGovernorProposal,
