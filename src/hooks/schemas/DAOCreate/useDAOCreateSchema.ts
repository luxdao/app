    ...
    () =>
      Yup.object().shape({
        ...
        erc20Token: Yup.object().when('essentials', {
          is: ({ governance }: DAOEssentials) => governance === GovernanceType.GOVERNOR_ERC20,
          then: () => erc20TokenShape,
        }),
        erc721Token: Yup.object().when('essentials', {
          is: ({ governance }: DAOEssentials) => governance === GovernanceType.GOVERNOR_ERC721,
          then: _schema =>
            _schema.shape({
              nfts: Yup.array()
                .min(1)
        ...
        governor: Yup.object().when('essentials', {
          is: ({ governance }: DAOEssentials) =>
            governance === GovernanceType.GOVERNOR_ERC20 ||
            governance === GovernanceType.GOVERNOR_ERC721,
          then: _schema =>
            _schema.shape({
